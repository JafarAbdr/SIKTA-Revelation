...
class Classseminartas extends CI_Controller_Modified {
	...
	public function index(){
		if($this->prosesOpen){
			switch($this->kodeForm){
				...
				case 3 :
					echo "0C";
					//$this->load->view("Classroom_room/Body_right/Seminartas/Langkah3"); 
	...
	public function getJSONDataSeminarTA1(){
		...
		$tempObjectDB =$this->controlSeminar->getAllDataHaveATime($this->tahunAk);
		if($tempObjectDB->getCountData() > 0){
			$data['kode'] = true;
			while($tempObjectDB->getNextCursor()){
				$tempMahasiswa = $controlMahasiswa->getAllData($tempObjectDB->getMahasiswa());
		...
		$tempObjectDB = $this->controlSidang->getAllDataHaveATime($this->tahunAk,"1");
		if($tempObjectDB->getCountData() > 0){
			$data['kode'] = true;
			while($tempObjectDB->getNextCursor()){
				$tempMahasiswa = $controlMahasiswa->getAllData($tempObjectDB->getMahasiswa());
	...
	//submit
	//<<--DISABLED
	public function setSeminarTA1next(){
	...
	public function setSeminarTA1(){
		...
		if(intval($ff[0]) == 1){
			echo "1";
			$this->load->view("Bodyright/Classroom/Successregistrasi",array("data"=> "Terima kasih, Data Anda berhasil dimasukan, status diproses seleksi dosen penguji")); 
		}else{
			echo "0".$ff[1];
		}
